...
<img src="/image/Carousel 깜빡임 이슈/구조.png" width="700" height="400" />
...
**Layer가 교차**되거나 새로 생기는 과정에서 깜빡임이 발생하는 것이었다.
...
3. 트랜지션이 완료되어 translate3d가 제거되면 다시 일반 GPU Layer -> Render Layer로 돌아가며 합성 과정을 다시 거치게 된다.
4. 이 때 새 탭 컴포넌트가 Render Layer로 복귀하면서 이미 GPU 레이어 위에서 동작 중인 Swiper와의 레이어 순서를 재조정한다.
